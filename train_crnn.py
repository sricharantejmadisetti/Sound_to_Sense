...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        model = BabyCryCRNN(num_classes=len(full_train_ds.CLASS_NAMES), pretrained=True)
        ...
        for name, param in model.named_parameters():
            if "fc" in name or "lstm" in name:
                ...
                param.requires_grad = True # Only fine-tune higher level semantic layers
                backbone_params.append(param)
            else:
                param.requires_grad = False # Freeze conv1, bn1, layer1, layer2 to control capacity
        ...
        checkpoint_path = f"checkpoints/crnn_fold_{fold}.pth"
