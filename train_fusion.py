...
def main():
    ...
    full_train_ds = BabyCryDataset(split="train_val", augment=True, extract_acoustic=True)
    full_val_ds = BabyCryDataset(split="train_val", augment=False, extract_acoustic=True)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        model = MultiFeatureFusionNet(num_classes=len(full_train_ds.CLASS_NAMES), pretrained=True)
        criterion = nn.CrossEntropyLoss(label_smoothing=0.15)
        
        # Differential learning rates with selective conv freezing
        backbone_params = []
        head_params = []
        for name, param in model.named_parameters():
            if "resnet_branch" in name and ("layer3" in name or "layer4" in name):
                param.requires_grad = True # Only fine-tune higher level conv layers
                backbone_params.append(param)
            elif "resnet_branch" in name:
                param.requires_grad = False # Freeze conv1, bn1, layer1, layer2
            else:
        ...
        ], weight_decay=2e-2) # Stronger weight decay to prevent overfitting
        
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
        
        checkpoint_path = f"checkpoints/fusion_fold_{fold}.pth"
